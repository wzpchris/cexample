...
int main(int argc, char** argv)
{
	...
	for(int i = 0; i < sizeof(a) / sizeof(int); ++i)
	{
		...
		int read_num = read_form_buffer(write_buffer, to_write_size);
		int last_num = zigzag_to_int(read_num);

		printf("%11d [%s]	==to-zigzag==>	%6d [%s]	==to-buf==>	%s 	read_num=%d	last_num=%d\n", 
				n, str, zz, str_zz, str_write, read_num, last_num);
	}
